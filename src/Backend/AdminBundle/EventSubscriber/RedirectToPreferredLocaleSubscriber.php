...
namespace Backend\AdminBundle\EventSubscriber;
...
class RedirectToPreferredLocaleSubscriber implements EventSubscriberInterface
{
    ...
    public function __construct(UrlGeneratorInterface $urlGenerator, $locales, $defaultLocale)
    {
        ...
        $this->locales = $locales[0];
        //print "<pre>";
        //var_dump($this->locales);
        //var_dump($defaultLocale );die;
        if (empty($this->locales)) {
            throw new \UnexpectedValueException('The list of supported locales must not be empty.');
        }

        $this->defaultLocale = $defaultLocale[0];
        //print "$this->defaultLocale";die;
    ...
    public function onKernelRequest(GetResponseEvent $event): void
    {
        ...
        //var_dump($preferredLanguage);die;
        if ($preferredLanguage !== $this->defaultLocale) {
            $response = new RedirectResponse($this->urlGenerator->generate('backend_admin_homepage', ['_locale' => $preferredLanguage]));
